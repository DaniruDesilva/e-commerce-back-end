import { creteProductDto } from "./dto/products.js";
...
export const getProducts = (req, res) => {
  ...
  if (req.query.categoryId) {
    ...
    const filledProducts = products.filter((product) => product.categoryId === categoryId)
    return res.status(200).json(filledProducts).send();
  }
...
export const createProduct = (req, res) => {
  //! We need to make sure that the data will always the correct format                              
    const product = creteProductDto.safeParse(req.body);

    if(!product.success) {
      return res.status(400).json({ message: `${product.error.message}`}).send();
    }

    product.push({categoryId: product.data.categoryId,
